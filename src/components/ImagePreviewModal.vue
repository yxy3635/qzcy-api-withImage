  ...
  <Teleport to="body">
    <div v-if="src" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/82 p-3 backdrop-blur-sm sm:p-4" @click.self="emit('close')">
      ...
      <Transition name="zoom-fade">
        <div v-if="zoomed" class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/94 p-3 backdrop-blur-md sm:p-5" @click.self="zoomed = false">
          <img :src="src" alt="放大预览图像" class="max-h-[calc(100dvh-24px)] max-w-full rounded-2xl object-contain shadow-[0_30px_120px_rgba(0,0,0,0.55)] sm:max-h-[94vh] sm:max-w-[96vw]" @click="zoomed = false" />
