...
import AdminRelayTrendChart from '@/components/admin/AdminRelayTrendChart.vue'
...
const relayCards = computed(() => [
  ...
  { label: '昨日 Tokens', value: Number(stats.value?.yesterdayRelayTokens || 0), previous: `今日 ${compact(stats.value?.todayRelayTokens)}`, accent: 'rose', icon: 'tokens', money: false },
  { label: '昨日调用金额', value: Number(stats.value?.yesterdayRelayCost || 0), previous: `今日 ${yuan(stats.value?.todayRelayCost)}`, accent: 'amber', icon: 'arrow-up', money: true }
])
...
<template>
  <AppLayout admin wide>
    <div class="dashboard-shell">
      ...
      <section class="relay-section">
        ...
        <div class="relay-grid">
          <article v-for="(card, index) in relayCards" :key="card.label" class="relay-card" :class="`accent-${card.accent}`" :style="{ '--delay': `${index * 65 + 180}ms` }">
            ...
            <strong><AnimatedNumber :value="card.value" :format="card.money ? yuan : compact" /></strong>
      ...
      <section class="relay-trend-board" aria-label="中转调用近 30 日趋势">
        <AdminRelayTrendChart :rows="stats?.relayDailyTrend || []" :days="30" :loading="loading" />
      </section>
...
.trend-board { margin-top: 1.35rem; }.relay-trend-board { margin-top: 1.5rem; }.relay-trend-board > * { min-width: 0; }.trend-board-heading { display: flex; align-items: flex-end; justify-content: space-between; gap: 1rem; padding: 0 .1rem; }.trend-board-heading h2 { margin-top: .38rem; font-size: 1.16rem; }.trend-board-heading > div > p:last-child { margin-top: .35rem; color: #617388; font-size: .76rem; font-weight: 650; }.trend-window { display: flex; flex: 0 0 auto; align-items: center; gap: .58rem; border: 1px solid rgba(255,255,255,.72); border-radius: .5rem; background: rgba(255,255,255,.52); padding: .55rem .7rem; color: #0369a1; box-shadow: 0 10px 26px rgba(30,73,94,.09), inset 0 1px 0 rgba(255,255,255,.84); backdrop-filter: blur(14px) saturate(135%); }.trend-window svg { width: 1.05rem; height: 1.05rem; fill: none; stroke: currentColor; stroke-width: 1.9; stroke-linecap: round; stroke-linejoin: round; }.trend-window strong, .trend-window small { display: block; }.trend-window strong { font-size: .72rem; font-weight: 850; }.trend-window small { margin-top: .08rem; color: #718196; font-size: .58rem; font-weight: 650; }.trend-grid { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: .85rem; margin-top: .8rem; }
