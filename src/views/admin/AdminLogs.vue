...
type RelayLogWithAliases = AdminRelayUsageLog & {
  user_agent?: string
  useragent?: string
}
...
function userAgentOf(log: AdminRelayUsageLog) {
  const aliases = log as RelayLogWithAliases
  const value = log.userAgent || aliases.user_agent || aliases.useragent || ''
  return value.trim() || '-'
}
          ...
          <table class="w-full min-w-[1480px] table-fixed text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100 font-semibold">
              ...
              <tr v-for="log in relayRecords" :key="log.id" class="align-top transition duration-200 hover:bg-sky-50/60" :class="relayLoading ? 'opacity-55' : 'opacity-100'">
                ...
                <td class="py-4"><p class="max-w-[170px] truncate font-black text-slate-900">{{ log.tokenName || '-' }}</p><p class="mt-1 max-w-[175px] truncate font-mono text-xs text-slate-500">{{ log.model || '-' }}</p><p class="mt-1 max-w-[175px] truncate text-xs text-slate-400">{{ log.channelName || '-' }}</p></td>
