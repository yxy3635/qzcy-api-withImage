...
管理员可在「注册设置」（`/admin/registration`）控制以下开关，保存后立即生效：

- **仅邀请码邀请注册**：新用户必须填写现有用户在「邀请返利」中生成的有效 6 位邀请码。
- **关闭注册**：暂停所有新用户注册和注册验证码发送，有邀请码也无法注册；已有用户登录和找回密码不受影响。

两个开关默认关闭，保持开放注册。同时开启时，「关闭注册」优先；重新开放后仍保留邀请码限制。注册页面加载失败时不会开放提交，服务端也会独立校验注册限制。

设置保存在 `registration_config` 表中。升级后重启后端会自动创建该表并初始化默认设置，之后的重启不会覆盖已保存的设置。
